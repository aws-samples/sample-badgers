...
<sub>🔵 **Home** | [Frontend](frontend/FRONTEND_README.md) | [Deployment](deployment/DEPLOYMENT_README.md) | [CDK Stacks](deployment/stacks/STACKS_README.md) | [Runtime](deployment/runtime/RUNTIME_README.md) | [S3 Files](deployment/s3_files/S3_FILES_README.md) | [Lambda Analyzers](deployment/lambdas/LAMBDA_ANALYZERS.md) | [Prompting System](deployment/s3_files/prompts/PROMPTING_SYSTEM_README.md) | [Vision LLM Theory](VISION_LLM_THEORY_README.md) | [Analyzer Wizard](frontend/ANALYZER_CREATION_WIZARD.md) | [Pricing Calculator](frontend/PRICING_CALCULATOR.md)</sub>
...
3. 🔧 **Agent selects tools** from 28+ specialized analyzers via MCP Gateway
4. ⚡ **Lambda analyzers** process document elements using Claude vision models
...
| ⚡ Compute          | AWS Lambda (28+ analyzer functions)                                |
...
| 🔗 `result_combiner`                  | Merge results from multiple analyses      |
...
| 🏛️ `cuneiform_analyzer`               | Ancient cuneiform script analysis         |
...
| 🔍 `strands_retrieve`                 | Memory retrieval for agent context        |
...
This deploys 9 CloudFormation stacks:
1. 📦 S3 (config + output buckets)
2. 🔐 Cognito (OAuth authentication)
3. 👤 IAM (execution roles)
4. ⚡ Lambda (28+ analyzer functions)
5. 🚪 Gateway (MCP endpoint)
...
BADGERS uses two Lambda layers shared across all 28+ analyzer functions:
...
- Includes dependencies: boto3, pillow, botocore
...
| `*claude-opus-4-5*`   | `CLAUDE_OPUS_PROFILE_ARN`   |
...
```bash
cd frontend
uv run python analyzer_wizard.py
```

The [Analyzer Creation Wizard](frontend/ANALYZER_CREATION_WIZARD.md) provides a guided UI that generates all required files automatically.
...
- [Anthropic's Claude in Amazon Bedrock](https://aws.amazon.com/bedrock/anthropic/) - Claude Opus 4.5, Sonnet 4.5, Haiku 4.5 hybrid reasoning models
...
- [Claude Opus 4.5 in Amazon Bedrock](https://aws.amazon.com/blogs/machine-learning/claude-opus-4-5-now-in-amazon-bedrock/) - Tool search, extended thinking, and agent capabilities
