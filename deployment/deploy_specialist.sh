#!/bin/bash
#
# Deploy a single specialist without redeploying the entire stack.
#
# Scans lambdas/code/ for specialist directories, presents an interactive
# picker, then runs: S3 sync → Lambda stack → Gateway stack.
#
# Usage: ./deploy_specialist.sh [--all]
    ...
    while true; do
        ...
        case "$key" in
            ...
            '') # Enter
                ...
                SELECTED_SPECIALIST="${SPECIALISTS[$selected]}"
                return
                ;;
            'q'|'Q')
                tput cnorm 2>/dev/null || true
...
validate_specialist() {
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        log_error "Specialist '${name}' is missing: ${missing[*]}"
        echo ""
        echo "  Required files:"
...
deploy_specialist() {
    ...
    # Upload only the files for this specialist + shared prompts
...
echo ""
echo -e "${BOLD}  Deploy Single Specialist${NC}"
echo -e "  ${DIM}Select an specialist to deploy (↑/↓ to move, Enter to select, q to quit)${NC}"
echo ""
...
echo ""
log_info "Selected: ${BOLD}${SELECTED_SPECIALIST}${NC}"
echo ""

# Validate
if ! validate_specialist "$SELECTED_SPECIALIST"; then
    exit 1
fi
...
echo ""
deploy_specialist "$SELECTED_SPECIALIST"
