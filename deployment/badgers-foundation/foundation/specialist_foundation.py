"""
Core specialist foundation class that orchestrates all foundation components.

This is the main class that specialist tools will use to perform analysis operations.
"""
...
def _get_specialist_config_path() -> str:
    """Get SPECIALIST_CONFIG_PATH by temporarily adding config module to path."""
    ...
    try:
        from config.config import (  # pylint: disable=import-outside-toplevel
            SPECIALIST_CONFIG_PATH as config_path,
        )
...
SPECIALIST_CONFIG_PATH = _get_specialist_config_path()
...
class SpecialistFoundation:
    """Core specialist functionality that all specific specialists inherit from."""
    ...
    def __init__(
        ...
        """
        Initialize the specialist foundation.

        Args:
            specialist_type: Type of specialist (e.g., 'diagram', 'table')
            config_path: Optional path to specialist config file. If None, tries local manifest first, then falls back to SPECIALIST_CONFIG_PATH
        ...
        try:
            ...
            self.logger = logging.getLogger(f"{__name__}.{specialist_type}")
            ...
            self.logger.info(
                "Initialized %s specialist with model %s",
                specialist_type,
            ...
            raise AnalysisError(
                f"Failed to initialize {specialist_type} specialist: {e}"
            ) from e
    ...
    def analyze(
        ...
        try:
            self.logger.info(
                "Starting %s analysis (audit_mode=%s)", self.specialist_type, audit_mode
            )
    ...
    def get_specialist_info(self) -> Dict[str, Any]:
        """
        Get information about this specialist instance.
    ...
    def validate_configuration(self) -> bool:
        """
        Validate that the specialist configuration is complete and valid.
    ...
    def _load_configuration(
        self, specialist_type: str, config_path: Optional[str]
        ...
        Returns:
            Tuple of (specialist_config, global_settings)
        """
        ...
        manifest_path = (
            foundation_dir / "tools" / f"{specialist_type}_specialist" / "manifest.json"
        )
        ...
        if manifest_path.exists():
            try:
                ...
                # Handle both single and multi-specialist manifests
                specialist_config = None
                if "specialist" in manifest:
                    # Single specialist manifest
                    specialist_config = manifest["specialist"]
                elif "specialists" in manifest:
                    # Multi-specialist manifest (e.g., metadata specialist)
                    # Try to find the matching specialist by type
                    if specialist_type in manifest["specialists"]:
                        specialist_config = manifest["specialists"][specialist_type]
                    else:
                        # Try to match by name
                        for _, config in manifest["specialists"].items():
                            if config.get("name") == f"{specialist_type}_specialist":
                ...
                if specialist_config:
                    ...
                    # Manifest is at tools/{specialist}/manifest.json
                    manifest_dir = manifest_path.parent

                    for path_key in [
                        "prompt_base_path",
                        "prompt_specialist_prompt_base_path",
                        "examples_path",
                        "wrapper_path",
                    ]:
                        if path_key in specialist_config:
                            path_value = specialist_config[path_key]
                            # Make absolute path from manifest directory
                            specialist_config[path_key] = str(
                    ...
                    try:
                        central_config_path = (
                            config_path if config_path else SPECIALIST_CONFIG_PATH
                        )
                    ...
                    return specialist_config, global_settings
            except Exception as e:
                self.logger.warning(
        ...
        self.config_path = config_path if config_path else SPECIALIST_CONFIG_PATH
        self.logger.info("Using central specialist config: %s", self.config_path)

        specialist_config = self.config_manager.get_specialist_config(
            specialist_type, self.config_path
        )
    ...
    def _get_dynamic_tokens_config(self) -> dict:
        """Load dynamic tokens config from specialist manifest."""
        default = {
            "weights": {
                "text_ratio": 0.2,
                "entropy": 0.3,
    ...
    def _finalize_result(self, result: str) -> str:
        ...
        try:
            ...
            formatted_result = self.response_processor.format_analysis_result(
                result, self.specialist_type, include_metadata=False
            )
