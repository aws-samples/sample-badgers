#!/bin/bash
# Deploy remediation_specialist container and update Lambda + IAM
# Usage: ./deploy_remediation_specialist.sh <deployment_id>
set -e
...
docker build \
  ...
  --provenance=false \
  --no-cache \
  -t "${ECR_URI}:remediation_specialist" \
  ./containers/remediation_specialist

# 4. Push
echo ""
echo "=== Pushing to ECR ==="
docker push "${ECR_URI}:remediation_specialist"
echo "✓ Image pushed"
...
aws lambda update-function-code \
  ...
  --image-uri "${ECR_URI}:remediation_specialist" \
  --region ${REGION}
echo "✓ Lambda updated"
