...
Input is page image bytes plus regions in normalized 0-1 page coordinates (the same
convention as image_enhancer's Region class). Output is one crop per region, resized so the
long side is the vision model's working size, with metadata stating exactly how much real
detail the crop contains.
...
SMALL_REGION_PX = 100        # below this (shorter side, source px) fine features are near the limit
...
class RegionResult:
    ...
    source_px_box: Optional[List[int]] = None  # padded region in page pixels: x1, y1, x2, y2
    source_px_size: Optional[List[int]] = None  # region w, h in page pixels (real information)
...
def _padded_box(r: RegionRequest, pad: float):
    x1, y1, x2, y2 = map(_clamp01, (r.x1, r.y1, r.x2, r.y2))
    if x2 <= x1 or y2 <= y1:
...
def inspect_regions(image_bytes: bytes, regions: List[RegionRequest],
                    image_uri: str = "",
                    pad: float = DEFAULT_PAD,
                    target_long_side: int = TARGET_LONG_SIDE) -> List[RegionResult]:
    ...
    for r in regions:
        result = RegionResult(
            region_id=r.region_id, image_uri=image_uri, task=r.task,
            flagged_by=r.flagged_by, concern=r.concern, page_px_size=[W, H],
        )
        try:
            box = _padded_box(r, pad)
        ...
        notes: List[str] = []
        if s > 1.0:
            notes.append(f"Enlarged {s:.1f}x by Lanczos interpolation; no detail beyond "
                         f"the {sw}x{sh} source pixels.")
        if min(sw, sh) < SMALL_REGION_PX:
            notes.append(f"Region is {sw}x{sh} source px. Features such as open vs closed loops "
