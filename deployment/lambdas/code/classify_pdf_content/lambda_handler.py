"""Classify PDF Content Lambda - S3-based config version with AgentCore Gateway session tracking."""
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Get image paths/data
        image_paths_str = body.get("image_paths")
        if not image_paths_str:
            raise ValidationError(
                message="Missing required parameter: image_paths",
        ...
        if not isinstance(image_paths, list):
            raise ValidationError(
                message="image_paths must be a JSON array",
        ...
        for idx, image_path in enumerate(image_paths):
            logger.info("Classifying page %d/%d", idx + 1, len(image_paths))

            # Get image data
            if image_path.startswith("data:") or len(image_path) > 1000:
                # Base64 encoded
                image_data = base64.b64decode(image_path.split(",")[-1])
            else:
                # Path - load from S3 or local
        ...
        logger.info(
            "Classification completed for %d pages (session: %s)",
            len(classifications),
            session_id,
        )
...
def _get_image_data(body: dict) -> bytes:
    ...
    raise ValidationError(
        message="Missing required parameter: image_data or image_path.",
        details={"provided_keys": list(body.keys())},
    )
