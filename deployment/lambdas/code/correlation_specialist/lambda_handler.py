"""Correlation Specialist Lambda - Correlates multiple specialist outputs into correlation_specialist XML.

This specialist takes S3 URIs of specialist XML outputs and correlates them into
a correlation_specialist XML structure using Opus 4.6 with adaptive thinking.
"""
...
def lambda_handler(
    ...
    try:
        ...
        specialist_uris = body.get("specialist_uris", [])
        ...
        # Fetch specialist outputs from S3
        full_text_content = _fetch_s3_content(s3, full_text_uri)

        specialist_contents = []
        for specialist in specialist_uris:
            content = _fetch_s3_content(s3, specialist["s3_uri"])
            specialist_contents.append(
                {
                    "name": specialist["specialist_name"],
                    "uri": specialist["s3_uri"],
                    "content": content,
                }
            )
        ...
        # Extract the correlation_specialist output from response
        ...
        logger.info(
            "Extracted correlation_specialist output length: %d",
        ...
        if not unified_document:
            ...
            return _error_response(
                "No correlation_specialist output generated - no text content in response"
            )
        ...
        logger.info(
            "After cleaning, correlation_specialist output length: %d",
            len(unified_document),
        )

        # Extract summary from the correlation_specialist output
...
def _load_config_from_s3(bucket: str, specialist_name: str) -> dict[str, Any]:
    """Load specialist configuration from S3 manifest."""
    manifest = load_manifest_from_s3(bucket, specialist_name)
...
def _load_correlation_prompts(s3, config_bucket: str, config: dict) -> str:
    ...
    for filename in prompt_files:
        key = f"prompts/correlation_specialist/{filename}"
        try:
...
def _build_user_message(
    ...
    specialist_contents: list,
    page_number: str,
    source_image_path: str,
) -> str:
    """Build the user message with all specialist outputs."""
    parts = [
        f"Correlate the following specialist outputs for page {page_number}.",
        f"Source image: {source_image_path}",
        "",
        "=== FULL TEXT SPECIALIST OUTPUT (CANONICAL SPINE) ===",
    ...
    for specialist in specialist_contents:
        parts.extend(
            [
                f"=== {specialist['name'].upper()} SPECIALIST OUTPUT ===",
                f"S3 URI: {specialist['uri']}",
                "",
                specialist["content"],
                "",
            ]
...
def _extract_summary(unified_document: str) -> str:
    """Extract summary from correlation_specialist XML."""
    import re

    match = re.search(r"<summary>(.*?)</summary>", unified_document, re.DOTALL)
    if match:
        return match.group(1).strip()
    return "Correlation completed. See correlation_specialist output for details."
...
def _error_response(message: str) -> dict:
    ...
    return {
        ...
        "body": json.dumps(
            {
                "correlation_specialist_uri": None,
