...
const CONTAINER_SPECIALISTS = new Set(['image_enhancer', 'remediation_specialist']);
...
function groupByCategory(specialists) {
  const groups = {};
...
export default function SpecialistSelector({ dirtyRef }) {
  ...
  const enabledCount = names.filter(n => specialists[n].enabled).length;
  const filterKey = filter.toLowerCase().replace(/ /g, '_');
  const groups = groupByCategory(specialists);

  const toggle = (name) => {
    setConfig(prev => ({
      ...prev,
      specialists: { ...prev.specialists, [name]: { ...prev.specialists[name], enabled: !prev.specialists[name].enabled } }
    }));
    setSaved(false);
  };
  ...
  const setAll = (enabled) => {
    setConfig(prev => {
      const updated = { ...prev.specialists };
      for (const n of names) updated[n] = { ...updated[n], enabled };
      return { ...prev, specialists: updated };
    });
    setSaved(false);
  };
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        Toggle specialists on/off to control which Lambda functions get deployed.
        Disabled specialists won't create Lambda functions or Gateway targets.
      ...
      {groups.map(([category, catNames]) => {
        ...
        const catEnabled = visible.filter(n => specialists[n].enabled).length;
        return (
          <div key={category} style={{ marginBottom: 16 }}>
            ...
            <div style={{
              ...
              {visible.map(name => {
                const enabled = specialists[name].enabled;
                const isContainer = CONTAINER_SPECIALISTS.has(name);
                return (
